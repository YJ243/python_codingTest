...
// dir 기준 0, 3이 대칭, 1, 2가 대칭이 되도록 설정하기
...
Marble Move(Marble marble){
    // tuple의 경우 다음과 같이 원하는 변수에 값을 뽑아줄 수 있음
    int x, y, dir;
    tie(x, y, dir) = marble;
    ...
    // Case 1: 벽이 없는 경우 그대로 한 칸 전진
    if(InRange(nx, ny))
        return make_tuple(nx, ny, dir);
    
    // Case 2: 벽이 있는 경우에는 방향을 반대로 틀어줌
    // 처음에 dx, dy를 dir 기준 0, 3이 대칭/ 1,2가 대칭이 되도록
    // 설정해놨기 때문에 간단하게 처리가 가능
    else
...
bool DuplicateMarbleExist(int target_idx){
    ...
    tie(target_x, target_y, ignor) = marbles[target_idx];

    return marble_cnt[target_x][target_y] >= 2;
}

// 충돌이 일어나는 구슬은 전부 지워주기
void RemoveDuplicateMarbles(){
    vector<Marble> temp_vector;

    // Step 2-1: 각 구슬의 위치에 count를 증가시킴
    for(int i=0; i<(int) marbles.size(); i++){
        int x, y;
    ...
    // Step 2-2: 충돌이 일어나지 않는 구슬만 전부 기록
    for(int i=0; i<(int)marbles.size(); i++)
        if(!DuplicateMarbleExist(i))
            temp_vector.push_back(marbles[i]);

    // Step 2-3: 나중을 위해 각 구슬의 위치에 적어놓은 count 수를 다시 초기화시키기
    for(int i=0; i<(int)marbles.size(); i++){
        int x, y;
        tie(x, y, ignore) = marbles[i];
        marble_cnt[x][y]--;
    ...
    // Step 2-4: 충돌이 이렁나지 않은 구슬들로 다시 채워주기
    marbles = temp_vector;
}
...
void Simulate(){
    ...
    // Step 2
    // 움직임 이후에 충돌이 일어나는 구슬들을 골라 목록에서 지워주기
