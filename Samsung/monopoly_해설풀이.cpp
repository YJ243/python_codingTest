 ...
 * 2024.03.03
 * 해설 풀이
 * 각 턴마다 모든 과정을 구현하는 데 O(N^2)의 연산이 필요
 * 최종 시간 복잡도는 O(T*N^2)이다.
*/
...
void Move(int x, int y){
    ...
    // Step 1. 현재 플레이어의 다음 위치와 방향을 구한다.
    int nx, ny, move_dir;
...
void Simulate(){
    // Step 1. next_player를 초기화한다.
    for(int i=0; i<n; i++)
        for(int j=0; j<n; j++)
            next_player[i][j] = EMPTY;

    // Step 2. 각 플레이어들을 한 칸씩 움직여준다.
    for(int i=0; i<n; i++)
        for(int j=0; j<n; j++)
            if(player[i][j] != EMPTY)
                Move(i,j);
    
    // Step 3. next_grid 값을 grid로 옮겨준다.
    for(int i=0; i<n; i++)
        for(int j=0; j<n; j++)
            player[i][j] = next_player[i][j];

    // Step 4. 남은 contract 기간을 1식 감소시킨다.
    for(int i=0; i<n; i++)
        for(int j=0; j<n; j++)
            if(contract[i][j] != EMPTY)
                DecContract(i,j);

    // Stpe 5. 새로운 contract를 갱신해준다.
    for(int i=0; i<n; i++)
        for(int j=0; j<n; j++)
            if(player[i][j] != EMPTY)
                AddContract(i,j);
