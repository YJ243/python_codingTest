 ...
 * 2024.02.20
 * 팩맨
 * 몬스터 수가 100만까지 늘어날 수 있기 때문에, (몬스터 위치, 바라보는 방향) 상태에 놓인 몬스터가
 * 몇 마리가 있는지를 관리하는 식으로 구현해야 한다.
 * 몬스터의 수는 많지만 격자 크기는 4*4로 작기 때문에 시뮬레이션을 진행하는 것이 좋음
 * 몬스터의 수가 많다고 해도 결국 (위치, 바라보고 있는 방향)이 일치하는 몬스터의 움직임은 정확히 동일하다
 * 따라서 동일한 위치에서 동일한 방향을 바라보고 있는 몬스터 자체를 관리하는 식으로 시뮬레이션을 진행할 수 있음
 * 이렇게 관리하게 되면 n*n 개의 위치에 대해 각 방향에 맞춰 이동시켜주는 것을 T초 동안 반복하면 됨
 * 따라서 시간복잡도를 O(TN^2)fh wnfdlf tn dlTdma
...
// 시체를 관리하기 위한 배열이다.
// (x,y) 위치에서 썩기 t초 전인 시체가 몇 개 있는지를 의미한다.
...
// 나아가려고 하는 위치가 영역 내에 있으며 몬스터 시체가 없고, 팩맨도 없다면
// 이동 가능하다.
...
tuple<int, int, int> GetNextPos(int x, int y, int move_dir){
    ...
    // 이동이 불가능하다면, 움직이지 않고 기존 상태 그대로 반환한다.
    return make_tuple(x, y, move_dir);
}

void MoveM(){
    // 각 (i,j) 칸에 k방향을 바라보고 있는 몬스터들이
    // 그 다음으로 이동해야할 위치 및 방향을 구해서
    // 전부 (칸, 방향) 단위로 이동시켜 준다.
    // 일일이 몬스터마다 위치를 구해 이동시키면 시간초과가 발생한다.
    for(int i=0; i<n; i++)
        for(int j=0; j<n; j++)
...
int GetKilledNum(int dir1, int dir2, int dir3){
    ...
    for(int i=0; i<3; i++){
        ...
        // 움직이는 도중에 격자를 벗어나는 경우라면, 선택되면 안된다.
        if(!InRange(nx ,ny))
            return -1;
...
void DoKill(tuple<int ,int, int> best_route){
    ...
    // 정해진 dir1, dir2, dir3 순서에 맞춰 이동하면서
    // 몬스터를 잡기
    int dirs[] = {dir1, dir2, dir3};
    for(int i=0; i<3; i++){
...
void Simulate(){
    // 매 초마다 기록하기 때문에 굳이 copy를 진행할 필요는 없다.
    // 각 칸에 있는 몬스터를 이동시킨다.
    MoveM();
