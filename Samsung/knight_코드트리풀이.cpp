/*
 * 2024.02.13
 * 총 시간복잡도: 기사별로 이동 가능 여부를 검사, 이동한 후의 상태를 갱신하는 과정을 Q번에 걸쳐 수행
 * O(QN^2), N은 기사의 수, Q는 명령의 수 
*/
#include <iostream>
#include <queue>
...
bool TryMovement(int idx, int dir){
    ...
    while(!q.empty()){
        ...
        // 다른 조각과 충돌하는 경우, 해당 조각도 같이 이동하기
        for(int i=1; i<=n; i++){
            if(is_moved[i] || k[i] <= 0)
                continue;
            if(r[i] > nr[x] + h[x]-1 || nr[x] > r[i] + h[i]-1)
                continue;
            if(c[i] > nc[x] + w[x]-1 || nr[x] > c[i] + w[i] -1)
                continue;
            is_moved[i] = true;
            q.push(i);
        }
