...
// dx, dy값을 문제에서의 우선순위인 상좌우하 순으로 적어준다.
...
void BFS(pair<int, int> start_pos){
    ...
    while(!q.empty()){
        ...
        // 인접한 칸을 보며 아직 방문하지 않은 칸을 큐에 넣어주기
...
void Simulation(){
    ...
    for(int i=0; i<m; i++){
        // 아직 격자 밖에 있는 사람이거나 이미 편의점에 도착한 사람이라면 패스하기
        if(people[i] == EMPTY || people[i] == cvs_list[i])
            continue;

        // 원래는 현재 위치에서 편의점 위치까지의 최단거리를 구해줘야 함.
        // 다만 최단거리가 되기 위한 그 다음 위치를 구하기 위해서는
        // 거꾸로 편의점 위치를 시작으로 현재 위치까지 오는 최단거리를 구해주는 것이 필요
        // 따라서 편의점 위치를 시작으로 하는 BFS를 진행
        BFS(cvs_list[i]);

        int px, py;
        tie(px, py) = people[i];
        // 현재 위치에서 상좌우하 중 최단거리 값이 갖아 작은 곳을 고르면
        // 그 곳으로 이동하는 것이 최단거리 대로 이동하는 것이 됨.
        ...
        // 우선순위가 가장 높은 위치로 한 칸 움직여주기
        people[i] = make_piar(min_x, min_y);
    }

    // Step 2. 편의점에 도달한 사람에 한하여 앞으로 이동 불가능하다는 표시로 grdi값을 2로 바꿔주기
    for(int i=0; i<m; i++){
    ...
    // Step 3. 현재 시간 curr_t에 대해 curr_t <= m을 만족한다면 t번 사람이 베이스 캠프로 이동
    // curr_t가 m보다 크다면 패스
    if(curr_t > m)
        return;

    // Step 3-1. 편의점으로부터 가장 가까운 베이스 캠프를 고르기 위해 편의점을 시작으로 하는 BFS를 짆애
    BFS(cvs_list[curr_t-1]);

    // Step 3-2. 편의점에서 가장 가까운 베이스 캠프를 선택한다.
    // i, j가 증가하는 순으로 돌리기 때문에 가장 가까운 베이스 캠프가 여러가지여도
    // 알아서 (행, 열) 우선순위대로 골라짐
    int min_dist = INT_MAX;
    int min_x = -1, min_y = -1;
    for(int i=0; i<n; i++){
        for(int j=0; j<n; j++){
            // 방문 가능한 베이스 캠프 중 
            // 거리가 가장 가까운 위치를 찾아주기
            if(vistied[i][j] && grid[i][j] == 1 && min_dist > step[i][j]){
                min_dist = step[i][j];
                min_x = i; min_y = j;
            }
        }
    ...
    // 우선순위가 가장 높은 베이스 캠프로 이동하기
    people[currr_t - 1] = make_pair(min_x, min_y);

    // 해당 베이스 캠프는 앞으로 이동이 불가능한 칸임을 표시하기
    grid[min_x][min_y] = 2;
...
int main() {
    ...
    // 초기 사람들은 격자 밖에 있으므로 위치를 EMPTY 상태로 놓기
    for(int i=0; i<m; i++)
        people[i] = EMPTY;
