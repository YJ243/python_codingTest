 ...
 * 2024.03.27
 * 해설 풀이
 * 시간복잡도: 술래는 매 턴 최대 3개의 도둑 중 하나를 잡게 되고, 이러한 과정을 최대 N
*/
...
tupel<int, int, int> GetNext(int x, int y, int move_dir){
    // 45도씩 8번 회전해보면서 최초로 이동 가능한 곳으로 움직인다.
    ...
    // 이동이 불가능하다면 현재 위치, 현재 방향 그대로 유지되어야 한다.
    return make_tupel(x, y, move_dir);
}
    ...
    for(int x=0; x<n; x++)
        for(int y=0; y<n; y++){
            ...
            if(piece_num == target_num){
                ...
                // 이동해야 할 위치와 바라보게 될 방향을 구한다.
                tie(next_x, next_y, next_dir) = GetNext(x, y, move_dir);
                // 현재 말의 방향을 바꿔준 뒤, 두 말의 위치를 교환한다.
...
// 현재 술래말의 위치가 (x, y),
// 바라보고 있는 방향이 d이고
// 지금까지 얻은 점수가 score일 때 
// 탐색을 계속 진행하는 함수이다.
void SearchMaxScore(int x, int y, int d, int score){
    // 더 이상 움직일 곳이 없다면 답을 갱신하고 퇴각한다.
    if(Done(x, y, d)){
        max_score = max(max_score, score);
        return;
    }

    // 현재 턴에 움직일 수 있는 곳을 전부 탐색한다.
    for(int dist = 1; dist <= n; dist++){
        int nx = x + dx[d]*dist, ny = y + dy[d]*dist;

        // 술래가 이동할 수 없는 위치라면, 패스한다.
        if(!TaggerCanGo(nx, ny))
            continue;

        // 더 탐색을 진행한 이후, 초기 상태로 다시 만들기 위해 temp 배열에 현재 board 상태를 저장해놓는다.
...
int main(){
    ...
    // 처음 (0, 0) 도둑말을 잡고, 모든 도둑말이 이동한 다음에 시작ㅎ나다.
