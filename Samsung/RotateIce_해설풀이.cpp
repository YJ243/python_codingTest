 ...
 * 회전하는 빙하
 * 해설 풀이
 * 총 Q번에 걸쳐 각 격자에 대해 회전, 녹이는 작업을 진행한 뒤 한 번 BFS를 통해 각 얼음 군집의 크기를 계산
 * 격자의 크기를 R이라고 했을 때 총 시간복잡도는 O(Q*R^2)
*/
...
void Melt(){
    // Step 1. 녹은 이후의 상태를 저장할 배열을 0으로 초기화한다.
    for(int i=0; i< grid_size; i++)
        for(int j=0; j<grid_size; j++)
            next_grid[i][j] = 0;
    
    // Stpe 2. 인접한 칸의 수가 3개 이하인 곳의 얼음을 찾아 1씩 녹인다.
    for(int i=0; i<grid_size; i++)
        for(int j=0; j<grid_size; j++){
            int cnt = GetNeighborNums(i,j);
            // Step 2-1. 녹는 경우에는 1을 빼서 넣어주기
            if(grid[i][j] && cnt < 3)
                next_grid[i][j] = grid[i][j] - 1;
            // Step 2-2. 녹지 않는 경우에는 그대로 넣어준다.
            else
                next_grid[i][j] = grid[i][j];
        }

    // Step 3. 녹은 이후의 결과를 다시 grid 배열로 가져온다.
    for(int i=0; i<grid_size; i++)
        for(int j=0; j<grid_size; j++)
            grid[i][j] = next_grid[i][j];
}
