...
for t in range(1, m+1):
    ...
    # 살아있는 포인트 중 루돌프에 가장 가까운 산타를 찾음
    for i in range(1, p+1):
        if not is_live[i]:
            continue
        
        # 튜플로 비교할 건데, 먼저는 거리 그 다음은 x가 큰 것, 그 다음은 y가 큰 것
    ...
    # 각 산타들은 루돌프와 가장 가까운 방향으로 한칸 이동함
    for i in range(1, p+1):
        ...
        if moveDir != -1:
            ...
            # 산타의 이동으로 충돌한 경우, 산타를 이동시키고 처리를 함
            if nx == rudolf[0] and ny == rudolf[1]:
                ...
                pos[i] = (nx,ny)
                board[nx][ny] = i
    # 라운드가 끝나고 탈락하지 않은 산타들의 점수를 1 증가시킴
    for i in range(1, p+1):
        if is_live[i]:
            points[i] += 1
# 결과를 출력
