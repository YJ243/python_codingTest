...
int n, m, p, c, d           // n: 격자 크기, m: 턴수, p: 산타 명수, c,d: 부딪혔을 떄의 거리와 점수
int board[MAX_N][MAX_N];    // 격자
pair<int, int> rudolf;      // 루돌프 위치

pair<int, int> pos[MAX_P];  // 각 산타의 위치
int stun[MAX_P];            // 기절 시간
int points[MAX_P];          // 각 산타의 점수
bool is_live[MAX_P];        // 산타가 살아있는지 여부
...
int main(){
    cin >> n >> m >> p >> c >> d;       // 변수들을 입력받기
    cin >> rudolf.first >> rudolf.second;
    board[rudolf.first][rudolf.second] = -1;    // 루돌프의 위치를 보드에 표시

    for(int i=1; i<=p; i++){
        int id;
        cin >> id;
        cin >> pos[id].first >> pos[id].second;
        board[pos[id].first][pos[id].second] = id;  // 각 산타의 위치를 보드에 표시
        is_live[id] = true;     // 산타가 살아있는지 여부를 표시
    }
    for(int t=1; t<=m; t++){
        int closestX = 10000, closestY = 10000, closestIdx = 0;

        // 살아있는 산타 중 루돌프에 가장 가까운 산타를 찾기
        for(int i=1; i<=p; i++){
